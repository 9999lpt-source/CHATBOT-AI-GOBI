...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def cancel_current_pipeline():
        ...
        if current_pipeline_task and not current_pipeline_task.done():
            ...
            try:
                ...
                print(
                    "🛑 [SERVER]: Đã ngắt chu trình xử lý trước đó.", flush=True
                )
    ...
    async def run_pipeline():
        try:
            ...
            if not audio_frames:
                print(
                    "⚠️ [STT]: Không nhận được dữ liệu âm thanh.", flush=True
                )
            ...
            # Chuyển sang STT
            print(
                "🤫 [STT]: Dừng thu âm, bắt đầu dịch giọng nói...",
                flush=True,
            )
            await send_signal(
            ...
            if not user_text or not user_text.strip():
                ...
                await send_signal(
                    "status",
                    {"message": "Tui chưa nghe rõ, ông nói lại nha!"},
                )
            ...
            await send_signal(
                "stop_sig", {"reason": "error", "message": str(e)}
            )
        ...
        while True:
            ...
            if "text" in message and message["text"]:
                ...
                if sig_type == "start_sig":
                    print(
                        "\n📩 [SIGNAL]: Nhận 'start_sig' -> Bắt đầu thu âm!",
                        flush=True,
                    )
                ...
                # --- 3. TÍN HIỆU HỦY KHẨN CẤP (NẾU CÓ) ---
                    ...
                    print(
                        "\n📩 [SIGNAL]: Nhận 'cancel_sig' -> Dừng toàn bộ!",
                        flush=True,
                    )
                    await cancel_current_pipeline()
                    await send_signal(
                        "stop_sig", {"reason": "user_cancelled"}
                    )
